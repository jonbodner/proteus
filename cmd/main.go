...
var productDaoCtxSqlite = ProductDaoCtx{}
var productDaoCtxPostgres = ProductDaoCtx{}
...
func init() {
	...
	err = proteus.ShouldBuild(ctx, &productDaoCtxSqlite, proteus.Sqlite)
	if err != nil {
		panic(err)
	}

	err = proteus.ShouldBuild(ctx, &productDaoCtxPostgres, proteus.Postgres)
	if err != nil {
		panic(err)
	}
}
...
func main() {
	...
	runCtx(setupDbSqlite, productDaoCtxSqlite)
	runCtx(setupDbPostgres, productDaoCtxPostgres)
}

func runCtx(setupDb setupDb, productDao ProductDaoCtx) {
	...
	*cost = 56.23
	...
	logger.Log(c, logger.DEBUG, fmt.Sprintln(productDao.Update(c, exec, p)))
	logger.Log(c, logger.DEBUG, fmt.Sprintln(productDao.FindByID(c, exec, 10)))
	logger.Log(c, logger.DEBUG, fmt.Sprintln(productDao.FindByNameAndCost(c, exec, "fred", 54.10)))
	logger.Log(c, logger.DEBUG, fmt.Sprintln(productDao.FindByNameAndCost(c, exec, "Thingie", 56.23)))

	//using a map of [string]interface{} works too!
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByIDMap(c, exec, 10))))
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByNameAndCostMap(c, exec, "Thingie", 56.23))))

	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByID(c, exec, 11))))
	m := map[string]interface{}{
		"Id":   11,
		"Name": "bobbo",
		"Cost": 12.94,
	}
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.UpdateMap(c, exec, m))))
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByID(c, exec, 11))))

	//searching using a slice
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByIDSlice(c, exec, []int{1, 3, 5}))))
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByIDSliceAndName(c, exec, []int{1, 3, 5}, "person1"))))
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByIDSliceNameAndCost(c, exec, []int{1, 3, 5}, "person3", nil))))
	logger.Log(c, logger.DEBUG, fmt.Sprintln((productDao.FindByIDSliceCostAndNameSlice(c, exec, []int{1, 3, 5}, []string{"person3", "person5"}, nil))))
